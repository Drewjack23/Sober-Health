...
import { Plus, Trash2, Save, Upload, Download, Calendar, LineChart, Salad, HeartPulse, Scale, TimerReset, Sun, Moon, ListChecks, CalendarDays, Target, Droplets } from "lucide-react";
...
function ProgressDashboard({ addictions, weightRows, bmi, weightGoal, waterLog, waterGoal }) {
  ...
  const waterPct = clamp(((waterLog?.amount || 0) / (waterGoal || 1)) * 100);
  ...
  return (
    <Section title="Goals & Progress" icon={<Target className="h-5 w-5" />} right={<div className="text-sm text-neutral-500">Editable goals below</div>}>
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Ring value={waterPct} label="Hydration" sublabel={`${waterLog?.amount || 0} / ${waterGoal} oz`} />
      </div>
    </Section>
  );
}
...
function AddictionTracker() {
  ...
  const setCost = (id, cost) => setItems(items.map(it => it.id === id ? { ...it, cost: parseFloat(cost) || 0 } : it));
          ...
          {items.map(it => (
            <motion.div key={it.id} layout className="rounded-2xl border border-neutral-200 dark:border-neutral-800 p-4 bg-white/80 dark:bg-neutral-900/80">
              <div className="flex items-start justify-between gap-3">
                ...
                {it.cost > 0 && <Pill tone="good">${Math.floor(daysBetween(it.resetAt) * (it.cost / 7)).toLocaleString()} saved</Pill>}
                  </div>
              ...
              <div className="text-sm text-neutral-500 mt-1 flex items-center gap-2">
                <span>Avg cost/week: $</span>
                <input type="number" className="input py-0.5 px-2 w-20 text-sm h-auto" value={it.cost || ""} onChange={e => setCost(it.id, e.target.value)} placeholder="0" />
              </div>
...
// ===== Water Tracker =====
function WaterTracker({ log, setLog, goal, setGoal, weight }) {
  const today = fmtDate(new Date());

  // Auto-reset if day changed
  useEffect(() => {
    if (log.date !== today) setLog({ date: today, amount: 0 });
  }, [today, log.date, setLog]);

  const add = (oz) => setLog(prev => ({ ...prev, date: today, amount: (prev.amount || 0) + oz }));
  const setRecommended = () => setGoal(Math.round(weight * 0.5));

  return (
    <Section title="Water Intake" icon={<Droplets className="h-5 w-5 text-blue-500" />} right={<div className="text-sm text-neutral-500">{(log.amount || 0)} / {goal} oz</div>}>
      <div className="flex flex-col sm:flex-row items-center gap-6">
        <div className="flex-1 w-full space-y-4">
          <div className="grid grid-cols-3 gap-2">
            {[8, 12, 16, 24, 32].map(oz => (
              <button key={oz} onClick={() => add(oz)} className="btn-subtle flex-col py-2 h-auto gap-1">
                <span className="font-bold">+{oz} oz</span>
                <span className="text-[10px] text-neutral-500">{oz === 8 ? "Cup" : oz === 16 ? "Bottle" : "Large"}</span>
              </button>
            ))}
            <div className="relative"><input type="number" placeholder="+Custom" className="input w-full h-full text-center text-sm" onKeyDown={e => { if (e.key === 'Enter') { add(parseInt(e.target.value) || 0); e.target.value = ''; } }} /></div>
          </div>
          <div className="flex items-center justify-between text-sm text-neutral-500 pt-2 border-t border-neutral-100 dark:border-neutral-800">
            <div className="flex items-center gap-2">Daily Goal: <input type="number" value={goal} onChange={e => setGoal(parseInt(e.target.value) || 64)} className="input w-16 py-1 text-center" /> oz</div>
            {weight > 0 && goal !== Math.round(weight * 0.5) && (
              <button onClick={setRecommended} className="text-blue-600 hover:underline text-xs">Set to {Math.round(weight * 0.5)} oz (Weight/2)</button>
            )}
          </div>
        </div>
      </div>
...
function DataBackup() {
  const exportAll = () => {
    const data = {
      ...
      waterLog: JSON.parse(localStorage.getItem("water:v1") || '{"date":"","amount":0}'),
      waterGoal: JSON.parse(localStorage.getItem("water:goal") || "64"),
    };
  ...
  const importAll = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (data.waterLog) localStorage.setItem("water:v1", JSON.stringify(data.waterLog));
...
export default function App() {
  ...
  // Water state
  const [waterLog, setWaterLog] = useLocalStorage("water:v1", { date: fmtDate(new Date()), amount: 0 });
  const [waterGoal, setWaterGoal] = useLocalStorage("water:goal", 64);
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(1200px_600px_at_20%_-10%,rgba(99,102,241,0.12),transparent),radial-gradient(1000px_500px_at_100%_20%,rgba(16,185,129,0.10),transparent)] dark:bg-[radial-gradient(1200px_600px_at_20%_-10%,rgba(99,102,241,0.12),transparent),radial-gradient(1000px_500px_at_100%_20%,rgba(16,185,129,0.06),transparent)] text-neutral-900 dark:text-neutral-100">
      ...
      <main className="mx-auto max-w-7xl p-4 md:p-6 space-y-6">
        ...
        <ProgressDashboard addictions={addictions} weightRows={rows} bmi={bmi} weightGoal={weightGoal} waterLog={waterLog} waterGoal={waterGoal} />
        ...
        <div className="grid xl:grid-cols-2 gap-6">
          <div className="space-y-6">
            <AddictionTracker />
            <WaterTracker log={waterLog} setLog={setWaterLog} goal={waterGoal} setGoal={setWaterGoal} weight={weight} />
          </div>
          <div className="space-y-6">
            <WeightTracker />
