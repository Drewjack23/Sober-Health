...
describe('meal safety and filtering', () => {
  ...
  it('excludes disliked ingredients and ranks liked foods', () => { const result = filterMeals(meals, { ...emptyOnboarding, likedFoods: ['salmon'], dislikedFoods: ['olives'] }); expect(result[0]?.id).toBe('salmon-bowl'); expect(result.every((meal) => !meal.ingredients.includes('olives'))).toBe(true); });
});
