...
export default function SignUpScreen() {
  ...
  const submit = async () => { const result = schema.safeParse({ firstName, email, password }); if (!result.success) { setError(result.error.issues[0]?.message ?? 'Check your details.'); return; } setLoading(true); setError(''); try { const response = await signUpWithEmail(email.trim(), password, firstName.trim()); const id = 'user' in response ? response.user?.id : undefined; createLocalAccount(email.trim(), firstName.trim(), { id }); router.replace('/onboarding/1'); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to create your account.'); } finally { setLoading(false); } };
  return <AuthScreen title="Create your account" subtitle="We’ll build a plan around your goals—not somebody else’s."><Field label="First name" placeholder="What should we call you?" value={firstName} onChangeText={setFirstName} /><Field label="Email" placeholder="you@example.com" autoCapitalize="none" keyboardType="email-address" value={email} onChangeText={setEmail} /><Field label="Password" placeholder="At least 8 characters" secureTextEntry value={password} onChangeText={setPassword} error={error} /><Text style={styles.terms}>By continuing, you agree to the <Text accessibilityRole="link" onPress={() => router.push('/terms')} style={styles.inlineLink}>Terms of Use</Text> and acknowledge the <Text accessibilityRole="link" onPress={() => router.push('/privacy-policy')} style={styles.inlineLink}>Privacy Policy</Text>.</Text><Button label="Continue" icon="arrow-forward" loading={loading} onPress={submit} /><Pressable onPress={() => router.replace('/sign-in')}><Text style={styles.center}>Already have an account? <Text style={styles.link}>Sign in</Text></Text></Pressable></AuthScreen>;
}
const styles = StyleSheet.create({ terms: { color: colors.muted, fontSize: 12, lineHeight: 18 }, inlineLink: { color: colors.purple, fontWeight: '600' }, link: { color: colors.purple, fontWeight: '800' }, center: { textAlign: 'center', color: colors.muted, marginTop: 4 } });
